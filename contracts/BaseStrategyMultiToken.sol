...
abstract contract BaseStrategyMultiToken {
    ...
    VaultAPI[NUM_TOKENS] public vaults;
     ...
     * @param _vaults The address of the Vault responsible for this Strategy.
    ...
    function _initialize(
        address[NUM_TOKENS] memory _vaults,
        address _strategist,
        address _rewards,
        address _keeper
    ) internal {
        ...
        for (uint256 i = 0; i < NUM_TOKENS; i++) {
            ...
            vaults[i].approve(rewards, uint256(-1)); // Allow rewards to be pulled
        }
    ...
    function governance() internal view returns (address) {
        return vaults[0].governance(); // Assume gov is the same for both vaults.
    }
     ...
     *
     *  See comments on `adjustPositions()`.
     *
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < NUM_TOKENS; i++) {
            ...
            uint256 outstanding = vaults[i].debtOutstanding();
            if (outstanding > debtThreshold) return true;

            // Check for profits and losses
            uint256 total = estimatedTotalAssets(wants[i]);
            // Trigger if we have a loss to report
            if (total.add(debtThreshold) < params.totalDebt) return true;

            uint256 profit = 0;
            if (total > totalDebt) profit = total.sub(totalDebt);
            if (total.add(debtThreshold) < params.totalDebt) return true;
            // Otherwise, only trigger if it "makes sense" economically (gas cost
            // is <N% of value moved)
            uint256 credit = vaults[i].creditAvailable();
            if (profitFactor.mul(callCost[i]) > credit.add(profit)) {
                return false;
            }
        }
    ...
    function harvest() external {
        ...
        for (uint256 i = 0; i < NUM_TOKENS; i++) {
            ...
            if (emergencyExit) {
                ...
                uint256 amountFreed = liquidateAllPositions(wants[i]);
                ...
                (profit, loss, debtPayment) = prepareReturn(i, debtOutstanding);
            }

            // Allow Vault to take up to the "harvested" balance of this contract,
            // which is the amount it has earned since the last time it reported to
            // the Vault.
            debts[i] = vaults[i].report(profit, loss, debtPayment);
    ...
    function withdraw(uint256 _amountNeeded) external returns (uint256 _loss) {
        uint256 vaultId = isOneOfTheVaults();
        // Liquidate as much as possible to `want`, up to `_amountNeeded`
        uint256 amountFreed;
        (amountFreed, _loss) = liquidatePosition(wants[vaultId], _amountNeeded);
        // Send it directly back (NOTE: Using `msg.sender` saves some gas here)
        SafeERC20.safeTransfer(wants[vaultId], msg.sender, amountFreed);
